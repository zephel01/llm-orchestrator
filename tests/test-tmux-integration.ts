...
async function execCommand(command: string): Promise<{ stdout: string; stderr: string; error: Error | null }> {
...
async function testCreateTeam() {
  ...
  const { error: createError } = await execCommand(
    `node ${cliPath} create ${teamName} --provider anthropic`
  );
...
async function testBasicTmux(teamName: string) {
...
async function testKillSession(sessionName: string) {
...
async function interactiveTest(teamName: string) {
