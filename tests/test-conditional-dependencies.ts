...
import { TaskStatus, ExecutionCondition } from '../src/dependencies/types.js';
import {
  normalizeDependency,
  evaluateCondition,
...
} from '../src/dependencies/condition-evaluator.js';
...
const dep2 = normalizeDependency({ taskId: 'task-b', condition: 'failure' as ExecutionCondition });
...
const dep3 = { taskId: 'task-a', condition: 'success' as ExecutionCondition };
...
const dep4 = { taskId: 'task-a', condition: 'failure' as ExecutionCondition };
...
const dep5 = { taskId: 'task-a', condition: 'any' as ExecutionCondition };
...
const dep6 = {
  taskId: 'task-a',
  condition: (result: any) => result.score > 0.8
};
...
const deps9 = [{ taskId: 'task-a', condition: 'failure' as ExecutionCondition }];
...
const dep10 = { taskId: 'task-a', condition: 'success' as ExecutionCondition };
...
const deps11: (string | { taskId: string; condition: ExecutionCondition })[] = ['task-a', { taskId: 'task-b', condition: 'failure' as ExecutionCondition }];
const mockDeps11 = (taskId: string) => {
  if (taskId === 'task-a') return { status: TaskStatus.COMPLETED };
